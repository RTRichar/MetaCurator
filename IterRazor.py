...
parser.add_argument('-e', '--HmmEvalue', required = False, default = 0.005, help = "\nEvalue threshold for hmm search\n")
parser.add_argument('-is', '--IterationSeries', required = False, default = '10,5,5,3,3', help = "\nThe IterationSeries and CoverageSeries arguments dictate the number of iterative searches to run and the minimum HMM coverege required for each round of extracting. This argument consists of a comma-separated list specifying the number of searches to run during each round of extraction. The list can vary in length, changing the total number of extraction rounds conducted. However, if a search iteration fails to yield any new reference sequence amplicons, IterRazor will break out of that round of searching and move to the next. By default, the software conducts four rounds of extraction with 10, 5, 5, 3 and 3 search iterations per round (-is 10,5,5,3,3).\n")
parser.add_argument('-cs', '--CoverageSeries', required = False, default = '1.0,0.98,0.95,0.9,0.65', help = "\nThis argument consists of a comma-separated list specifying the minimum HMM coverage required per round for an extracted amplicon reference sequence to be added to the reference sequence fasta. The list can vary in length but must be equal in length to the IterationSeries list. By default, the software conducts four rounds of extraction with coverage limits of 1.0, 0.98, 0.95, 0.9 and 0.65 for each respective search round (-cs 1.0,0.98,0.95,0.9,0.65).\n")
